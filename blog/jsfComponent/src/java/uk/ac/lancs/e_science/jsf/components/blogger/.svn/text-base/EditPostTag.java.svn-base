...
public class EditPostTag extends UIComponentTag {
	...
	public String getComponentType() {
		return "uk.ac.lancs.e_science.jsf.components.blogger.EditPost";
	}
	/**
	...
	protected void setProperties(UIComponent component){
		...
		if (controller!=null){
			if (isValueReference(controller)){
				FacesContext context = FacesContext.getCurrentInstance();
				Application app = context.getApplication();
				ValueBinding vb = app.createValueBinding(controller);
				component.setValueBinding("controller",vb);
			}else{
				component.getAttributes().put("controller",controller);
			}
		}		
	}
