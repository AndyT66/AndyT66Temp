...
<sakai:view title="#{msgs.title}">
	...
	<h:form>
          ...
          <h:outputText styleClass="spanPageTitle" value="#{msgs.listOfSiteMembers}" />
	      <f:verbatim><div style="height: 20px;"></div></f:verbatim>             
          <h:dataTable id="result" styleClass="listHier speciallistHier" value="#{sakaiBean.siteMembers}" var="member">
	          <h:column>
                      <h:commandLink id="userId"
                        title="#{member.userDisplayId}"
                        action="#{postListViewerController.doShowPostOfMember}">
                        <h:outputText value="#{member.userDisplayId}"/>
                    </h:commandLink>
                 </h:column>
             </h:dataTable>
	</h:form>
