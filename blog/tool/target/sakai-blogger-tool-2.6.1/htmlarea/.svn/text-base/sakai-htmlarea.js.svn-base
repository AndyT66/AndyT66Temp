// MODIFIED VERSION. CENTRE-OF E-SCIENCE. LANCASTER UNIVERSITY. UNITED KINGDOM
// AUTHOR OF CHANGES: MIGUEL GONZALEZ LOSA.
//
/**********************************************************************************
*
...
else
{
	document.msg_formattedtextnotsupported = "<br>The rich text editor is not supported by the browser you are using.<br>" 
		+ "Please see <a href='/sakai-blogger-tool/htmlarea/reference.html#supportedbrowsers' target='_blank'>"
...
else
{
	...
	if (document.htmlareas == undefined)
	{
		...
		_editor_url = "/sakai-blogger-tool/htmlarea/";
		_editor_lang = "en";
		
		// funky way to include the HTMLArea JavaScript library, from within JavaScript
		document.write('<script type="text/javascript" src="/sakai-blogger-tool/htmlarea/htmlarea.js"></script>');
...
function chef_setupformattedtextarea(textarea_id, width, height, toolbar_num_button_rows,onChange,onSubmit)
{
	chef_setupformattedtext(textarea_id, width, height, toolbar_num_button_rows, false, onChange,onSubmit);
}


// textarea_id - The HTML id of the HTML textarea 
// that should be turned into a full-featured HTML document editing widget
function chef_setuphtmldocumenttextarea(textarea_id, width, height, toolbar_num_button_rows,onChange,onSubmit)
{
	chef_setupformattedtext(textarea_id, width, height, toolbar_num_button_rows, true,onChange,onSubmit);
}


function chef_setupformattedtext(textarea_id, width, height, toolbar_num_button_rows, fulldocumentediting,onChange,onSubmit)
{
	...
	editor.setFunctionOnChange(onChange);
	editor.setFunctionOnSubmit(onSubmit);
	if (fulldocumentediting)
	{
